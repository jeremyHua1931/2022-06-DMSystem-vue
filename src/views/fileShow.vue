<template>
  ...
  <el-table :data="FileList" border style="width: 100%" highlight-current-row="true">
    ...
    <el-table-column label="日期">
      <template #default="scope">
        <span style="margin-left: 10px  text-aligned=left" @dblclick="JumpTo(scope.row.Title)"
              show-overflow-tooltip="true">{{ scope.row.ModifyDate }}</span>
      </template>
    </el-table-column>
    <el-table-column label="操作" align="center" fixed="right" width="280">
      <template #default="scope">
        ...
        <el-button size="small"
                   @click="EditFormVisible = true;EditTo(scope.row.Title,scope.row.ObjectType,scope.row.fileid) "
                   v-if="scope.row.fileid!='fix'">编辑
        </el-button>
        <el-divider direction="vertical" v-if="scope.row.fileid!='fix'"></el-divider>
        <el-popconfirm
...
console.log("第一次打开", Paths.UserId)
